...
char *readln()
{
    ...
    while (1) {
        ...
        if (c == 9) { /* Tab completion */
            ...
            if (best_match[0] != '\0') {
                int old_len_word = strlen(tmp);
                int new_len_word = strlen(best_match);
...
char *cmdline;
{
    ...
    int cmd_count = 0;
    int pipes[19][2];  /* MAXARGS-1 = 31, but use 19 for safety in K&R C */
    int i, j;
    char *cmd;
